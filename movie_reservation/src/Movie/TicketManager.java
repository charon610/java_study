...
public class TicketManager {
	...
	public void add() {
		...
		for (int i = 0; i < tm.getPos(); i++) {
			if (tm.getMT()[i].getMovie().getTitle().equals(title)
					&& tm.getMT()[i].getScreen().getName().equals(screen)) {
				switch1 = true;
		...
		for (int i = 0; i < pos; i++) {
			if (tickets[i].getTicketNo().equals(ticketNo)) {
				...
				for (int j = 0; j < tickets[i].getSeats().length - 1; j++) {
					System.out.print(
							tickets[i].getSeats()[j].getRowNo() + "열 " + tickets[i].getSeats()[j].getSeatNo() + "번, ");
				}
